...
const FileUploader = class {
  constructor(hostRef) {
    ...
    this.fwStageChanged = index.createEvent(this, "fwStageChanged", 7);
    /**
          ...
          if (xhr.status === 200) {
            ...
            this.setFile(fileId, {
              error: this.fileUploadError ||
                Translation.TranslationController.t('fileUploader.fileUploadError'),
            });
    ...
    if (this.accept) {
      ...
      if (!isPassed) {
        errors.push(this.acceptError ||
          Translation.TranslationController.t('fileUploader.acceptError'));
      }
    }
    if (this.maxFileSize !== 0) {
      if (fileSize > this.maxFileSize * 1024 * 1024) {
        ...
        errors.push(this.maxFileSizeError ||
          Translation.TranslationController.t('fileUploader.maxFileSizeError'));
      }
    }
    ...
    if (files.length <= this.filesLimit) {
      ...
      this.errors = [
        this.maxFilesLimitError ||
          Translation.TranslationController.t('fileUploader.maxFilesLimitError'),
      ];
      passed = false;
    }
    if (passed) {
   ...
   */
  renderFiles() {
    return (index.h("div", { class: 'files', key: 'files' }, index.h("div", { class: 'files-center' }, index.h("div", { class: 'files-title' }, Translation.TranslationController.t('fileUploader.selectedFiles')), this.files.map((file) => (index.h("fw-file-uploader-file", { fileId: file.id, name: file.name, onFwRemoveFile: (event) => {
  ...
  render() {
    ...
    index$1.renderHiddenField(this.host, this.name, null, this._getFiles());
    return (index.h("div", { class: 'file-uploader-container' }, index.h("input", Object.assign({ type: 'file', name: this.name, hidden: true }, multipleFiles, { style: { display: 'none' }, onChange: (ev) => this.fileHandler(ev), ref: (el) => (this.fileInputElement = el) })), this.renderFileUploader()));
  }
...
exports.fw_file_uploader = FileUploader;
