...
const DataTable = class {
  constructor(hostRef) {
    ...
    this.fwSelectionChange = index.createEvent(this, "fwSelectionChange", 7);
    this.fwSelectAllChange = index.createEvent(this, "fwSelectAllChange", 7);
    /**
  ...
  componentDidLoad() {
    if (this.showSettings) {
      ...
      this.popperInstance = popper.createPopper(this.settingsButton, this.settings, TABLE_POPPER_CONFIG);
    }
  }
  /**
  ...
  renderKebabMenu(rowData) {
    ...
    if (kebabMenuProps) {
      return (index.h("fw-kebab-menu", Object.assign({ variant: this.rowActionsMenuVariant }, kebabMenuProps)));
    }
    return null;
  }
  /**
  ...
  renderWebComponent(componentName, props) {
    ...
    if (window.customElements.get(componentName)) {
      ...
      template = index.h(WebComponentTag, Object.assign({}, props), slotText);
    }
    else {
      template = null;
    }
  ...
  renderCustomTemplate(customTemplate, cellValue) {
    return customTemplate(index.h, cellValue);
  }
  /**
   * private
  ...
  renderPredefinedVariant(columnVariant, cellValue) {
    ...
    if (columnVariant === 'anchor') {
      template = index.h("fw-custom-cell-anchor", Object.assign({}, cellValue));
    }
    else if (columnVariant === 'user') {
      template = index.h("fw-custom-cell-user", Object.assign({}, cellValue));
    }
    else if (columnVariant === 'icon') {
      template = index.h("fw-custom-cell-icon", Object.assign({}, cellValue));
    }
    else if (columnVariant === 'paragraph') {
      template = (index.h("fw-custom-cell-paragraph", Object.assign({}, cellValue)));
    }
    else {
      template = null;
    }
  ...
  renderTableHeader() {
    ...
    return this.orderedColumns.filter((column) => !column.hide).length ? (index.h("tr", { role: 'row' }, this.orderedColumns.length && this.isSelectable && (index.h("th", { ref: (el) => (this.selectColumnHeader = el), key: 'isSelectable', "aria-colindex": 1, style: { width: '40px' } }, this.isAllSelectable && (index.h("fw-checkbox", { id: 'select-all', value: 'select-all', checked: selectAllChecked, onFwChange: (event) => { var _a, _b; return this.selectAllRows((_b = (_a = event.detail) === null || _a === void 0 ? void 0 : _a.meta) === null || _b === void 0 ? void 0 : _b.checked); } })))), this.orderedColumns.map((column, columnIndex) => {
      let textAlign = null;
      if (column.textAlign &&
        !(column.variant &&
      ...
      return (index.h("th", Object.assign({ role: 'columnheader', key: column.key, "aria-colindex": this.isSelectable ? columnIndex + 2 : columnIndex + 1, class: { hidden: column.hide }, style: headerStyles }, optionalAttrs), column.customHeader
        ? this.renderCustomTemplate(column.customHeader, column.text)
        : column.text));
    }), this.rowActions.length !== 0 && (index.h("th", { class: 'row-actions', role: 'columnheader', "aria-colindex": this.isSelectable
      ...
      ? Translation.TranslationController.t('datatable.actions')
  ...
  renderTableBody() {
    return this.orderedColumns.filter((column) => !column.hide).length
      ? this.rows.map((row, rowIndex) => {
        return (index.h("tr", { key: row.id, role: 'row', class: {
            active: this.selected.includes(row.id),
            loading: this.rowsLoading[row.id] ? true : false,
          }, "aria-rowindex": rowIndex + 1 }, this.orderedColumns.length && this.isSelectable && (index.h("td", { class: 'data-table-checkbox', "aria-colindex": 1, "data-has-focusable-child": 'true' }, index.h("fw-checkbox", { value: row.id ? row.id : '', checked: this.selected.includes(row.id), onFwChange: (event) => {
            var _a, _b;
          ...
          return (index.h("td", Object.assign({ role: 'gridcell', class: { hidden: orderedColumn.hide }, style: cellStyle }, attrs), this.renderTableCell(orderedColumn, row[orderedColumn.key])));
        }), this.rowActions.length !== 0 && (index.h("td", { class: 'row-actions', "data-has-focusable-child": 'true', "aria-colindex": this.isSelectable
            ? this.orderedColumns.length + 2
          ...
          : this.rowActions.map((action) => {
            ...
            if (!action.hideForRowIds ||
              ...
              const buttonSize = iconProps
                ? 'icon-small'
                : 'small';
              actionTemplate = (index.h("fw-tooltip", { content: action.name, distance: '5' }, index.h("fw-button", { tabIndex: 0, size: buttonSize, color: 'secondary', onKeyUp: (event) => (event.code === 'Space' ||
                  event.code === 'Enter') &&
                  this.performRowAction(action, row), onClick: () => this.performRowAction(action, row), "aria-label": action.name }, iconProps ? (index.h("fw-icon", Object.assign({}, iconProps))) : (action.name))));
            }
   ...
   */
  renderTableSettings() {
    return (index.h("div", { class: 'table-settings' }, index.h("div", { class: 'table-settings-container' }, index.h("button", { class: 'table-settings-button', tabIndex: 0, ref: (el) => (this.settingsButton = el), onClick: () => this.toggleSettings(!this.isSettingsOpen), onKeyDown: (ev) => ev.key === 'Enter' && this.toggleSettings(!this.isSettingsOpen), disabled: this.isLoading }, index.h("fw-icon", { name: 'settings', library: 'system', size: 16 })), index.h("div", { ref: (el) => (this.settings = el), class: {
        'table-settings-options': true,
        'show': this.isSettingsOpen,
      } }, index.h("div", { class: 'table-settings-header' }, index.h("span", { class: 'title' }, Translation.TranslationController.t('datatable.customizeColumns')), index.h("button", { class: 'reset', tabIndex: 0, ref: (el) => (this.settingsResetButton = el), onClick: () => {
        this.resetSettings();
      } }, "Reset")), index.h("div", { class: 'table-settings-content' }, index.h("div", { class: 'table-settings-content-left' }, index.h("div", { class: 'table-settings-content-search' }, index.h("fw-input", { "icon-left": 'search', placeholder: 'Search', ref: (el) => (this.settingsInput = el), onFwInput: (event) => this.settingsSearch(event.detail.value), onFwInputClear: (event) => this.settingsSearch(event.detail.value), "clear-input": true })), index.h("div", { class: 'table-settings-content-choose' }, index.h("div", { class: 'table-settings-content-title' }, Translation.TranslationController.t('datatable.chooseColumns')), index.h("div", { class: 'table-settings-content-checkboxes' }, this.columnsHideSetting.map((column) => {
      ...
      if (showColumn) {
        dragItemTemplate = (index.h("div", { key: column.key }, index.h("fw-checkbox", { value: column.key, onFwChange: (event) => {
            var _a, _b;
    ...
    })))), index.h("div", { class: 'table-settings-content-right' }, index.h("div", { class: 'table-settings-content-title' }, index.h("span", null, "Selected columns "), index.h("span", null, "(", this.columnsDragSetting.filter((setting) => !setting.hide)
      .length, ")")), index.h("div", { class: 'table-settings-content-draggable' }, this.columnsDragSetting
      .filter((column) => column.lock)
      .map((column) => {
      let lockTemplate = null;
      lockTemplate = (index.h("div", { key: column.key, class: 'table-settings-drag-item', "data-column-key": column.key }, index.h("div", { class: 'table-settings-drag-item-icon non-drag' }, index.h("fw-icon", { name: 'lock', size: 11 })), index.h("div", { class: 'table-settings-drag-item-text', title: column.text }, column.text)));
      return lockTemplate;
    }), index.h("fw-drag-container", { ref: (el) => (this.settingsDragContainer = el), onFwDrop: (event) => this.settingsColumnDrop(event.detail.droppedElement.dataset.columnKey, event.detail.droppedIndex) }, this.columnsDragSetting
      .filter((column) => !column.lock)
      .map((column) => {
      let dragTemplate = null;
      if (!column.hide) {
        dragTemplate = (index.h("div", { key: column.key, class: 'table-settings-drag-item', "data-column-key": column.key, draggable: true }, index.h("div", { class: 'table-settings-drag-item-icon' }, index.h("fw-icon", { library: 'system', name: 'drag', size: 11 })), index.h("div", { class: 'table-settings-drag-item-text', title: column.text }, column.text), index.h("button", { class: 'table-settings-drag-item-close', tabIndex: 0, onKeyDown: (event) => event.key === 'Enter' &&
            this.settingsColumnToggle(column.key, false, event), onClick: () => this.settingsColumnToggle(column.key, false) }, index.h("fw-icon", { library: 'system', name: 'cross-big', size: 7 }))));
      }
      return dragTemplate;
    }))))), index.h("div", { class: 'table-settings-footer' }, index.h("fw-button", { color: 'secondary', tabIndex: 0, id: 'close-settings', onClick: () => {
        this.resetSettings();
        this.toggleSettings(false);
      } }, "Cancel"), index.h("fw-button", { color: 'primary', tabIndex: 0, id: 'apply-settings', ref: (el) => (this.settingsUpdateButton = el), onClick: () => this.applySettings() }, "Update")))), this.isSettingsOpen && (index.h("div", { role: 'presentation', class: 'table-settings-overlay', onClick: () => this.toggleSettings(false) }))));
  }
  /**
   * private
  ...
  renderTableShimmer() {
    ...
    if (columnsLength) {
      ...
      for (let index$1 = 1; index$1 <= shimmerCount; index$1++) {
        shimmerTemplate.push(index.h("tr", null, Array(columnsLength)
          .fill(1)
          .map(() => {
          return (index.h("td", null, index.h("fw-skeleton", { height: '12px' })));
        })));
      }
    }
   ...
   */
  render() {
    return (index.h("div", { class: 'fw-data-table-container' }, index.h("div", { class: {
        'fw-data-table-scrollable': true,
        'loading': this.isLoading || this.showShimmer,
        'shimmer': this.showShimmer,
      }, ref: (el) => (this.tableContainer = el) }, index.h("table", { class: {
        'fw-data-table': true,
        'is-selectable': this.isSelectable,
      }, role: 'grid', "aria-colcount": this.orderedColumns.length, "aria-label": this.label }, index.h("thead", null, this.renderTableHeader()), index.h("tbody", null, this.showShimmer
      ? this.renderTableShimmer()
      : this.renderTableBody()))), this.showSettings && this.renderTableSettings(), (this.isLoading || this.showShimmer) && (index.h("div", { class: 'fw-data-table--loading' }))));
  }
...
exports.fw_data_table = DataTable;
